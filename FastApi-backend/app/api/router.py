...
# ----------------------------------------------------
# ML MODEL EVALUATION ENDPOINTS (Person A ML Evaluation)
# ----------------------------------------------------

@router.get("/model/evaluation", summary="Get Unified ML Evaluation Summary")
async def get_evaluation_summary_endpoint() -> Dict[str, Any]:
    """Returns consolidated evaluation metrics and artifact statuses for all ML models."""
    from app.ml.evaluation import generate_evaluation_summary
    return generate_evaluation_summary(save_artifacts=True)


@router.get("/model/evaluation/xgboost", summary="Get XGBoost Conflict Classifier Evaluation")
async def get_xgboost_evaluation_endpoint() -> Dict[str, Any]:
    """Returns detailed evaluation metrics, confusion matrix, breakdown, and visual artifact paths for XGBoost."""
    from app.ml.evaluation import evaluate_conflict_classifier
    return evaluate_conflict_classifier(save_artifacts=True)



@router.get("/model/evaluation/yolov8", summary="Get YOLOv8 Vision Model Evaluation Status")
async def get_yolov8_evaluation_endpoint() -> Dict[str, Any]:
    """Returns evaluation metrics, dataset status, and confusion matrix availability for YOLOv8 intrusion detector."""
    from app.ml.evaluation import evaluate_yolo_detector
    return evaluate_yolo_detector(save_artifacts=True)
